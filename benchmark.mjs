...
async function main() {
  ...
  for (let i = 1; i <= ROW_COUNT; i++) {
    ...
    const balance =
      (seedRandom(i * 7) % 100000) + (seedRandom(i * 13) % 100) / 100;
  ...
  // --- UPDATE complex ---
  {
    ...
    const balances = Array.from(
      { length: ITERATIONS },
      (_, i) => (seedRandom(i * 23) % 100000) + 0.5,
    );

    for (let i = 0; i < WARMUP; i++) {
      sSt.executeSync([balances[i], 27, 28]);
      lSt.run(balances[i], 27, 28);
    }
  ...
  // --- DELETE complex ---
  {
    ...
    const lSt = ldb.prepare(
      "DELETE FROM users WHERE age >= ? AND age <= ? AND active = 1",
    );
  ...
  for (let i = 1; i <= ROW_COUNT * 3; i++) {
    ...
    const amount =
      (seedRandom(i * 19) % 990) + 10 + (seedRandom(i * 23) % 100) / 100;
  ...
  console.log("\n" + "=".repeat(80));
  console.log(
    `SCORE: Stoolap ${stoolapWins} wins  |  SQLite ${sqliteWins} wins`,
  );
  console.log("");
  console.log("NOTES:");
