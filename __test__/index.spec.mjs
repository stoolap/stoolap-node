...
describe("DDL and DML", () => {
  ...
  it("should delete rows and return changes count", async () => {
    await db.execute("INSERT INTO users VALUES ($1, $2, $3, $4)", [
      99,
      "Temp",
      0,
      null,
...
describe("Queries", () => {
  ...
  before(async () => {
    ...
    `);
    await db.execute("INSERT INTO products VALUES ($1, $2, $3, $4)", [
      1,
      "Widget",
      9.99,
      true,
  ...
  it("should query with params", async () => {
    const rows = await db.query("SELECT * FROM products WHERE price > $1", [
      10.0,
    ]);
    assert.equal(rows.length, 1);
  ...
  it("should queryOne return null for no results", async () => {
    const row = await db.queryOne("SELECT * FROM products WHERE id = $1", [
      999,
    ]);
...
describe("Type conversions", () => {
  ...
  it("should handle null values", async () => {
    await db.execute("INSERT INTO types (id, int_val) VALUES ($1, $2)", [
      1,
      null,
    ]);
  ...
  it("should handle integer values", async () => {
    ...
    const row = await db.queryOne("SELECT int_val FROM types WHERE id = $1", [
      2,
    ]);
  ...
  it("should handle float values", async () => {
    ...
    const row = await db.queryOne("SELECT float_val FROM types WHERE id = $1", [
      3,
    ]);
  ...
  it("should handle text values", async () => {
    await db.execute("INSERT INTO types (id, text_val) VALUES ($1, $2)", [
      4,
      "hello world",
    ]);
    const row = await db.queryOne("SELECT text_val FROM types WHERE id = $1", [
      4,
    ]);
  ...
  it("should handle boolean values", async () => {
    await db.execute("INSERT INTO types (id, bool_val) VALUES ($1, $2)", [
      5,
      true,
    ]);
    await db.execute("INSERT INTO types (id, bool_val) VALUES ($1, $2)", [
    ...
    const r1 = await db.queryOne("SELECT bool_val FROM types WHERE id = $1", [
      5,
    ]);
    const r2 = await db.queryOne("SELECT bool_val FROM types WHERE id = $1", [
      6,
    ]);
  ...
  it("should handle JSON values", async () => {
    ...
    const row = await db.queryOne("SELECT json_val FROM types WHERE id = $1", [
      7,
    ]);
...
describe("Transaction", () => {
  ...
  it("should handle transaction helper pattern", async () => {
    ...
    const rows = await db.query(
      "SELECT * FROM tx_test WHERE id IN ($1, $2)",
      [10, 11],
    );
...
describe("Error handling", () => {
  ...
  before(async () => {
    db = await Database.open(":memory:");
    await db.exec(
      "CREATE TABLE err_test (id INTEGER PRIMARY KEY, name TEXT NOT NULL)",
    );
  });
  ...
  it("should throw on NOT NULL violation", async () => {
    await assert.rejects(
      () =>
        db.execute("INSERT INTO err_test (id, name) VALUES ($1, $2)", [
          1,
          null,
        ]),
      (err) => {
...
describe("Advanced queries", () => {
  ...
  it("should handle JOINs", async () => {
    ...
    await db.execute("INSERT INTO departments VALUES ($1, $2, $3)", [
      1,
      "Engineering",
      "SF",
    ]);
...
describe("Concurrency", () => {
  ...
  it("should keep the event loop responsive during async queries", async () => {
    const db = await Database.open(":memory:");
    await db.exec(
      "CREATE TABLE conc_slow (id INTEGER PRIMARY KEY, val INTEGER)",
    );
    ...
    try {
      const rows = await db.query(
        "SELECT COUNT(*) AS cnt FROM conc_slow a, conc_slow b",
      );
      assert.equal(rows.length, 1);
    ...
    assert.ok(
      ticks > 0,
      "expected timers to keep firing while async query was running",
    );
  });
});
...
describe("Sync methods", () => {
  ...
  it("should querySync with named params", () => {
    const rows = db.querySync("SELECT * FROM sync_test WHERE name = :name", {
      name: "Bob",
    });
    assert.equal(rows.length, 1);
  ...
  it("should queryOneSync with named params", () => {
    const row = db.queryOneSync("SELECT * FROM sync_test WHERE id = :id", {
      id: 2,
    });
    assert.ok(row);
...
describe("Batch execution", () => {
  ...
  before(async () => {
    db = await Database.open(":memory:");
    db.execSync(
      "CREATE TABLE batch_test (id INTEGER PRIMARY KEY, name TEXT, val INTEGER)",
    );
  });
  ...
  it("should executeBatchSync for deletes", () => {
    ...
    const result = db.executeBatchSync("DELETE FROM batch_test WHERE id = $1", [
      [100],
      [101],
    ]);
...
describe("PreparedStatement sync", () => {
  ...
  before(async () => {
    db = await Database.open(":memory:");
    db.execSync(
      "CREATE TABLE ps_sync (id INTEGER PRIMARY KEY, name TEXT, active BOOLEAN)",
    );
  });
  ...
  it("should support named params with prepared statements synchronously", () => {
    const insert = db.prepare(
      "INSERT INTO ps_sync VALUES (:id, :name, :active)",
    );
    const result = insert.executeSync({
      id: 20,
      name: "NamedSync",
      active: true,
    });
...
describe("Transaction sync", () => {
  ...
  it("should queryOneSync within sync transaction", () => {
    ...
    const missing = tx.queryOneSync("SELECT * FROM tx_sync WHERE id = $1", [
      999,
    ]);
  ...
  it("should executeBatchSync within transaction", () => {
    ...
    const rows = db.querySync(
      "SELECT * FROM tx_sync WHERE id >= $1 ORDER BY id",
      [10],
    );
    assert.equal(rows.length, 3);
...
describe("Named parameters", () => {
  ...
  it("should update with named params", async () => {
    ...
    assert.equal(r.changes, 1);
    const row = await db.queryOne(
      "SELECT score FROM named_test WHERE id = $1",
      [3],
    );
  ...
  it("should delete with named params", async () => {
    ...
    const r = await db.execute("DELETE FROM named_test WHERE id = :id", {
      id: 99,
    });
    assert.equal(r.changes, 1);
  });
  ...
  it("should not rewrite named params inside SQL string literals", async () => {
    const row = await db.queryOne(
      "SELECT ':city' AS literal_value, :city AS bound_value",
      {
        city: "NYC",
      },
    );
  ...
  it("should use positional params in async transaction", async () => {
    const tx = await db.begin();
    await tx.execute("INSERT INTO named_test VALUES ($1, $2, $3, $4)", [
      50,
      "TxPos",
      "LA",
      50.0,
    ]);
    const row = await tx.queryOne("SELECT * FROM named_test WHERE id = $1", [
      50,
    ]);
    assert.ok(row);
  ...
  it("should use positional params in sync transaction", () => {
    const tx = db.beginSync();
    tx.executeSync("INSERT INTO named_test VALUES ($1, $2, $3, $4)", [
      51,
      "SyncPos",
      "CHI",
      60.0,
    ]);
    const row = tx.queryOneSync("SELECT * FROM named_test WHERE name = $1", [
      "SyncPos",
    ]);
    assert.ok(row);
...
describe("Extended type conversions", () => {
  ...
  it("should handle undefined as NULL", async () => {
    await db.execute("INSERT INTO ext_types (id, text_val) VALUES ($1, $2)", [
      1,
      undefined,
    ]);
    const row = await db.queryOne(
      "SELECT text_val FROM ext_types WHERE id = $1",
      [1],
    );
  ...
  it("should handle BigInt parameter", async () => {
    await db.execute("INSERT INTO ext_types (id, int_val) VALUES ($1, $2)", [
      2,
      BigInt(123456789),
    ]);
    const row = await db.queryOne(
      "SELECT int_val FROM ext_types WHERE id = $1",
      [2],
    );
  ...
  it("should handle Date parameter as TIMESTAMP", async () => {
    ...
    await db.execute("INSERT INTO ext_types (id, ts_val) VALUES ($1, $2)", [
      3,
      date,
    ]);
    const row = await db.queryOne(
      "SELECT ts_val FROM ext_types WHERE id = $1",
      [3],
    );
  ...
  it("should handle negative numbers", async () => {
    ...
    const row = await db.queryOne(
      "SELECT int_val, float_val FROM ext_types WHERE id = $1",
      [4],
    );
    assert.equal(row.int_val, -42);
    assert.ok(Math.abs(row.float_val - -3.14) < 0.001);
  ...
  it("should handle Object parameter as JSON", async () => {
    ...
    await db.execute("INSERT INTO ext_types (id, json_val) VALUES ($1, $2)", [
      9,
      obj,
    ]);
    const row = await db.queryOne(
      "SELECT json_val FROM ext_types WHERE id = $1",
      [9],
    );
    const parsed = JSON.parse(row.json_val);
...
describe("Persistence", () => {
  it("should persist multiple tables and types across restart", async () => {
    ...
    try {
      ...
      `);
      await db.execute("INSERT INTO kv VALUES ($1, $2, $3)", [
        1,
        "greeting",
        "hello",
      ]);
      ...
      await db.execute("INSERT INTO nums VALUES ($1, $2, $3)", [
        2,
        2.71,
        false,
      ]);
  ...
  it("should create volumes directory (not snapshots) on persist", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "stoolap-vol-"));
    const dbPath = path.join(tmpDir, "vol.db");
    try {
      const db = await Database.open(dbPath);
      await db.exec("CREATE TABLE t (id INTEGER PRIMARY KEY, val TEXT)");
      await db.execute("INSERT INTO t VALUES ($1, $2)", [1, "test"]);
      await db.exec("PRAGMA CHECKPOINT");
      await db.close();

      // v0.4.0: volumes/ should exist, snapshots/ should not
      assert.ok(
        fs.existsSync(path.join(dbPath, "volumes")),
        "volumes/ directory should exist",
      );
      assert.ok(
        !fs.existsSync(path.join(dbPath, "snapshots")),
        "snapshots/ directory should not exist",
      );

      // Verify data survived
      const db2 = await Database.open(dbPath);
      const row = await db2.queryOne("SELECT * FROM t WHERE id = $1", [1]);
      assert.equal(row.val, "test");
      await db2.close();
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  ...
  it("should persist transaction commit but not rollback", async () => {
    const tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "stoolap-tx-persist-"),
    );
...
describe("Error handling extended", () => {
  ...
  before(async () => {
    db = await Database.open(":memory:");
    db.execSync(
      "CREATE TABLE err_ext (id INTEGER PRIMARY KEY, name TEXT NOT NULL)",
    );
  });
  ...
  it("should throw on sync NOT NULL violation", () => {
    assert.throws(
      () =>
        db.executeSync("INSERT INTO err_ext (id, name) VALUES ($1, $2)", [
          1,
          null,
        ]),
      (err) => {
...
describe("Edge cases", () => {
  ...
  it("should handle multiple columns with same value types", async () => {
    ...
    await db.execute("INSERT INTO multi_col VALUES ($1, $2, $3, $4)", [
      "w",
      "x",
      "y",
      "z",
  ...
  it("should handle all NULL row", async () => {
    await db.exec(
      "CREATE TABLE all_null (a TEXT, b INTEGER, c FLOAT, d BOOLEAN)",
    );
    await db.execute("INSERT INTO all_null VALUES ($1, $2, $3, $4)", [
      null,
      null,
      null,
      null,
...
describe("Vector support", () => {
  ...
  it("should insert multiple vectors", async () => {
    await db.execute(
      "INSERT INTO embeddings (id, embedding) VALUES (2, '[0.4, 0.5, 0.6]')",
    );
    await db.execute(
      "INSERT INTO embeddings (id, embedding) VALUES (3, '[0.7, 0.8, 0.9]')",
    );
  ...
  it("should return vectors as Float32Array", async () => {
    ...
    assert.ok(
      ...
      "embedding should be Float32Array",
    );
  ...
  it("should handle NULL vectors", async () => {
    ...
    const row = await db.queryOne(
      "SELECT embedding FROM embeddings WHERE id = 4",
    );
  ...
  it("should work with higher-dimensional vectors", async () => {
    await db.exec(
      "CREATE TABLE hd_vecs (id INTEGER PRIMARY KEY, vec VECTOR(128))",
    );
    const dims = 128;
  ...
  it("should accept Float32Array as bind parameter", async () => {
    await db.exec(
      "CREATE TABLE vec_params (id INTEGER PRIMARY KEY, vec VECTOR(3))",
    );
    const vec = new Float32Array([1.0, 2.0, 3.0]);
    await db.execute(
      "INSERT INTO vec_params (id, vec) VALUES (1, '[1.0, 2.0, 3.0]')",
    );
  ...
  it("should support vectors in transactions", async () => {
    await db.exec(
      "CREATE TABLE tx_vecs (id INTEGER PRIMARY KEY, vec VECTOR(3))",
    );
    const tx = await db.begin();
    await tx.execute(
      "INSERT INTO tx_vecs (id, vec) VALUES (1, '[1.0, 2.0, 3.0]')",
    );
    await tx.execute(
      "INSERT INTO tx_vecs (id, vec) VALUES (2, '[4.0, 5.0, 6.0]')",
    );
  ...
  it("should reject wrong dimension count on insert", async () => {
    await assert.rejects(
      db.execute(
        "INSERT INTO embeddings (id, embedding) VALUES (99, '[0.1, 0.2]')",
      ),
  ...
  it("should support HNSW index creation", async () => {
    await db.exec(
      "CREATE TABLE hnsw_test (id INTEGER PRIMARY KEY, vec VECTOR(3))",
    );
    await db.execute(
      "INSERT INTO hnsw_test (id, vec) VALUES (1, '[0.1, 0.2, 0.3]')",
    );
    await db.execute(
      "INSERT INTO hnsw_test (id, vec) VALUES (2, '[0.4, 0.5, 0.6]')",
    );
    await db.execute(
      "INSERT INTO hnsw_test (id, vec) VALUES (3, '[0.7, 0.8, 0.9]')",
    );
